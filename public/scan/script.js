...
function init() {
  ...
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    showError(
      "Your browser does not support camera access. Please try using image upload or update your browser."
    );
...
function startScanner() {
  ...
  if (!isSecureContext) {
    showError(
      "Camera access requires HTTPS. Please use a secure connection or try scanning from an image instead."
    );
    return;
  }
  ...
  navigator.mediaDevices
    ...
    .catch((err) => {
      ...
      if (err.name === "NotAllowedError") {
        ...
        errorMsg =
          "No camera found. Please connect a camera or try scanning from an image.";
      } else if (err.message && err.message.includes("HTTPS")) {
        errorMsg =
          "Camera access requires a secure connection (HTTPS). Try scanning from an image instead.";
      }
  ...
  reader.onload = function (e) {
    img.onload = function () {
      ...
      if (code) {
        ...
        showError(
          "No QR code found in the image. Please try a different image or use the camera scanner."
        );
      }
    };
